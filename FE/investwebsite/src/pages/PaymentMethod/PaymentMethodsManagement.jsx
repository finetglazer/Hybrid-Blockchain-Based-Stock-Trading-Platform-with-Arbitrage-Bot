...
const PaymentMethodsManagement = () => {
    ...
    const onClickCategorySummary = (className, item, metadataClassNamePrefix, index) => {
        const expandIcon = document.querySelector(`.${className}`).querySelector(".expand-icon");
        ...
        if (!categoryWrapper.classList.contains("details-show")) {
            ...
            root.render(<ItemDetails item={item} metadataClassNamePrefix={metadataClassNamePrefix} index={index} onClickMetadata={onClickMetadata} />);
    ...
    const onClickMetadata = (className, metadata) => {
        ...
        if (metadataWrapper.classList.contains("details-show")) {
            ...
            const metadataDetails = metadataWrapper.querySelector(".metadata-details");
    ...
    return (
        <div className="container payment-method-management-container">
            <Breadcrumb
                ...
                items={[
                    {
                        title: <span style={{color: "rgba(255, 255, 255, 0.6)"}}>Account dashboard</span>,
                        href: '/account-dashboard',
                    },
                    {
                        title: <span style={{color: "rgba(255, 255, 255, 0.6)"}}> {">"} </span>,
                    },
                    {
                        title: <span style={{color: "rgba(255, 255, 255, 1)"}}>Payment methods</span>,
            ...
            <div className="body">
                ...
                {!loading && !error && !bankAccounts.length && !creditCards.length && !debitCards.length && !digitalWallets.length &&
                ...
                {props.items.map((item, index) => {
                    ...
                    return (
                        <div className={"category-wrapper " + `${prefix}-${index}`}>
                            <div className="summary" onClick={() => props.onClick(`${prefix}-${index}`, item, prefix, index)}>
                                ...
                                <div className="item-status">
                                    {item.status === "ACTIVE" && <img src="../../../src/assets/tick.png" alt="status icon" />}
                                    {item.status === "INACTIVE" && <img src="../../../src/assets/x.png" alt="status icon"/>}
                                    {item.status === "VERIFICATION_PENDING" && <img src="../../../src/assets/pending.png" alt="status icon"/>}
                                    <p className="status" style={{color: item.status === "ACTIVE" ? "springgreen"
                                            : (item.status === "INACTIVE" ? "red" : "gray")}}>{item.status}</p>
                                </div>
                            </div>
                        </div>
                    )
...
const ItemDetails = ({item, metadataClassNamePrefix, index, onClickMetadata}) => {
    return (
        <>
            <div className="details-info">
                ...
                <div className="properties">
                    ...
                    <div className={`property metadata ${metadataClassNamePrefix}-metadata-wrapper-${index}`} >
                        <div className="title" onClick={() => onClickMetadata(`${metadataClassNamePrefix}-metadata-wrapper-${index}`, item.metadata)}>
                            <span className="label">Metadata</span>
                            <button className="expand-icon"><img src="../../../src/assets/right-arrow.png" alt="expand icon" /></button>
                        </div>
            ...
            <div className="crud-btns">
                ...
                <div className="btn-wrapper">
                    <p className="description">Update information about this payment method</p>
